...
// ----------------------------------------------------------------------------
// RUAS — construir máscara (buffer padrão, sem estender) e clippá-la
// ----------------------------------------------------------------------------

/** Buffer padrão (caps arredondados) a partir de LineString/MultiLineString em WGS84. */
...
/** Apenas máscara (sem AOI) — buffer padrão (caps arredondados). */
export function buildRuaRestrictionMask(ruas = [], defaultWidth = 12) {
    const buffers = [];
    for (const r of ruas) {
        try {
    ...
    return buffers.length === 1 ? buffers[0] : unionAll(buffers);
}

/** Máscara de ruas CLIPPADA pela AOI (robusta + teimosa + planar WGS). */
export function buildRuaRestrictionMaskClipped(
    ruas = [],
    aoiWgs,
    defaultWidth = 12
) {
    const mask = buildRuaRestrictionMask(ruas, defaultWidth);
    if (!mask) { DBG("clipMask: sem máscara"); return null; }
    if (!aoiWgs) { DBG("clipMask: sem AOI"); return mask; }

    // 1) robusto sem shrink (evita sumir na borda)
    let out = clipToAoiWgsRobusto(mask, aoiWgs, {
        gridDecimals: 1,
        shrinkMeters: 0,
    });

    // 2) teimoso (cresce AOI e volta a limitar)
    if ((!out || getArea(out) <= 1e-9)) {
        try {
            if (turf.booleanIntersects(mask, aoiWgs)) {
                DBG("clipMask: usando 'teimoso' (expande AOI se necessário)");
                out = clipToAoiWgsTeimoso(mask, aoiWgs, { gridDecimals: 1 });
            }
        } catch { }
    }

    // 3) fallback planar sem projeção (Martinez direto em WGS)
    if ((!out || getArea(out) <= 1e-9)) {
        DBG("clipMask: fallback planar WGS (martinez sem projeção)");
        out = clipToAoiWgs_PlanarNoProj(mask, aoiWgs);
    }

    // 4) último recurso: turf.intersect direto em WGS
    if ((!out || getArea(out) <= 1e-9)) {
